    ...
    return (
        <div className="flex flex-col items-center gap-10 p-20">
            ...
            <div className="backdrop-blur-md bg-white/10 flex flex-col items-center justify-center p-2 rounded-full">
                <IconCard icon={<Handshake size={30} onClick={() => {setHandIconClicked(true); setTvIconClicked(false)}} />} bgColor={handIconClicked ? '#FF3D00' : undefined} rounded='rounded-t-full' />
                <IconCard icon={<TvMinimal size={30} onClick={() => {setTvIconClicked(true); setHandIconClicked(false)}} />} bgColor={tvIconClicked ? '#FF3D00' : undefined} rounded='rounded-b-full' />
            </div>
